...
## v2.10.0 (2026/07/19) <Badge text="最新版" type="tip"/>

### Feature

- 新增任务「分享查看页」：可分享链接实时查看收集情况，支持访问密码、表单字段展示与脱敏、文件字段脱敏、名单 Tab 配置
- 个人中心钱包支持支付宝充值
- 任务表单支持「快速新建字段」：粘贴一段文字一次性批量生成多个字段
...
- 支持设置任务密码，提交页可对带密码任务快捷切换
- 文件页支持自动下载归档与一键复制文件链接
- 归档下载点击后立即在下载历史展示「归档中」占位记录
- 批量下载与下载选中任务时自动打开下载历史面板
- 管理后台日志查询支持时间范围筛选
- 新增 task/grouped 聚合接口，修复 no-task 虚拟分类 404
- 文件管理页筛选条件持久化到 URL query，刷新或分享链接可保留筛选状态

### Bugfix

- 任务列表改为按分类合并接口，避免 GET /api/task 报错
- 任务配置页改用单任务接口替代全量列表请求
- 任务卡片缓存按 recent 维度区分，避免最近提交记录缺失
- 兼容任务批注图片和模板文件跨存储模式访问
- 本机模式批注图片上传后立即回填预览地址
- 名单手动添加改为接口批量入库，补齐模板文件和批注图片本机上传链路
- 修复文件管理页刷新后分类下拉显示 id 而非名称的问题
- 修复「存储空间」卡片在极小用量时显示 0% 的问题
- 修复用户用量统计与快速上传限制逻辑，已删除文件不再计入用量
- 修复用户已用空间为 0 时误判超限导致无法上传的问题
- 提交、撤回、批量删除文件后即时刷新用户用量缓存，避免空间占用展示滞后
- 修复用户概览缓存的竞态写入问题，确保用量数据准确
